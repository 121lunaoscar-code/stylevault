    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({ model:"claude-sonnet-4-20250514", max_tokens:1000,
          system:"Eres experto asesor de moda. Responde SOLO en JSON válido sin markdown. JSON: outfit (array: emoji,name,why), explanation (string reglas moda).",
          messages:[{ role:"user", content:`Armario:\n${list}\n\nEvento: ${selEv}\nTemporada: ${selSe}\n\nCrea el outfit ideal y explica las reglas de moda.` }]
        })
      });
  ...
  const sendChat = async (msg) => {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        method:"POST", headers:{ "Content-Type":"application/json" },
        body: JSON.stringify({ model:"claude-sonnet-4-20250514", max_tokens:1000,
          system:"Eres StyleVault, asesor de moda experto y sofisticado. Explicas reglas de moda, colores, siluetas, dress codes. Siempre en español.",
          messages: next.map(x => ({ role: x.role==="assistant"?"assistant":"user", content: x.text }))
        })
      });
      const d = await res.json();
      setMsgs([...next, { role:"assistant", text: d.content.map(i=>i.text||"").join("") }]);
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({ model:"claude-sonnet-4-20250514", max_tokens:1400,
          system:"Eres experto en moda y retail global. Responde SOLO en JSON válido sin markdown. JSON: intro (string), items (array: name, description, priceRange, stores (array strings), hasAmazon (boolean), tip, why).",
          messages:[{ role:"user", content:`Recomienda ropa "${shopCat}" con presupuesto ${bLabel} ${locCtx}${shopSt?`, estilo ${shopSt}`:""}.  Usa tiendas reales: Zara, H&M, Mango, Shein, ASOS, Amazon, Nike, Adidas, Uniqlo, Gap, Primark, Pull&Bear, Bershka, Liverpool, Palacio de Hierro. Precios realistas en USD y moneda local.` }]
        })
      });
      const d = await res.json();
      setShopR(JSON.parse(d.content.map(i=>i.text||"").join("").replace(/```json|```/g,"").trim()));
      ...
      <div style={{ width:"100%", maxWidth:"380px" }}>
        ...
        <div className="card" style={{ padding:"28px" }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:"11px" }}>
            ...
            {lerr && <div style={{ color:"#f87171", fontSize:"12px", textAlign:"center", lineHeight:1.5 }}>{lerr}</div>}
