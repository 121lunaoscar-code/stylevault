...
- Moda sostenible y marcas éticas
...
- Si el usuario tiene prendas en su armario y preguntas sobre combinaciones, úsalas
- Termina con una pregunta de seguimiento relevante para continuar la conversación
- Responde SIEMPRE en español
- Nunca uses prefijos como "Asesor:" o "StyleVault:"`;

const OUTFIT_SYSTEM = `Eres experto asesor de moda. Responde SOLO en JSON válido sin markdown ni backticks. 
JSON con: outfit (array de objetos: emoji, name, why), explanation (string con reglas de moda aplicadas), colorPalette (string describiendo la paleta de colores del look).`;
...
.btn-success:hover { background: rgba(39,174,96,.2); }
...
.inp:focus { border-color: rgba(196,151,63,.4); box-shadow: 0 0 0 3px rgba(196,151,63,.05); }
...
.card-gold:hover { border-color: rgba(196,151,63,.3); }
...
/* ── Chat bubbles ── */
.bubble-user {
  background: rgba(196,151,63,.08);
  border: 1px solid rgba(196,151,63,.15);
...
.photo-box:hover { border-color: rgba(196,151,63,.3); background: rgba(196,151,63,.03); }
...
export default function StyleVault() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      setProfile(p);
      setScreen(p.plan === "Admin" ? "admin" : "app");
    } catch { setLerr("Error de conexión. Intenta de nuevo."); }
    setAL(false);
  };
  ...
  const handleRegister = async () => {
    ...
    } catch { setLerr("Error al crear cuenta. Intenta de nuevo."); }
    setAL(false);
  };
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      ...
      setLerr(res.ok ? "✅ Revisa tu correo para recuperar tu contraseña." : "Error al enviar. Intenta de nuevo.");
  ...
  const handlePhoto = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    r.onload = (ev) => setPp(ev.target?.result as string);
    r.readAsDataURL(f);
  };
  ...
  const generateOutfit = async () => {
    ...
    try {
      const raw = await callClaude(OUTFIT_SYSTEM, [
        { role: "user", content: `Armario:\n${list}\n\nEvento: ${selEv}\nTemporada: ${selSe}\n\nCrea el outfit ideal con las prendas del armario y explica las reglas de moda aplicadas.` },
      ]);
    ...
    } catch { setOR({ outfit: [], explanation: "Error al generar. Intenta de nuevo.", colorPalette: "" }); }
    setOL(false);
  };
  ...
  const searchShop = async () => {
    ...
    try {
      const raw = await callClaude(SHOP_SYSTEM, [
        { role: "user", content: `Recomienda ropa "${shopCat}" con presupuesto ${bLabel} ${locCtx}${shopSt ? `, estilo ${shopSt}` : ""}. Usa tiendas reales: Zara, H&M, Mango, Shein, ASOS, Amazon, Nike, Adidas, Uniqlo, Gap, Primark, Liverpool, Palacio de Hierro. Precios realistas en USD y moneda local.` },
      ]);
      ...
      <div style={{ fontFamily: "'Jost', sans-serif", background: "#080808", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px" }}>
        ...
        <div style={{ width: "100%", maxWidth: "360px" }}>
          ...
          <div style={{ textAlign: "center", marginBottom: "48px" }}>
            <div style={{ fontSize: "11px", letterSpacing: "6px", color: "#3a3632", marginBottom: "12px", textTransform: "uppercase" }}>✦</div>
            <div className="serif" style={{ fontSize: "42px", letterSpacing: "10px", color: "#C4973F", fontWeight: 300 }}>
              STYLE<em>VAULT</em>
            </div>
          ...
          <div className="card">
            ...
            <div style={{ display: "flex", marginBottom: "28px", borderBottom: "1px solid #1c1c1c" }}>
              {["login", "register"].map((m) => (
                <button
                  key={m}
                  onClick={() => { setLmode(m); setLerr(""); }}
                  style={{ flex: 1, padding: "11px", background: "none", border: "none", borderBottom: `2px solid ${lmode === m ? "#C4973F" : "transparent"}`, color: lmode === m ? "#C4973F" : "#3a3632", cursor: "pointer", fontFamily: "'Jost', sans-serif", fontSize: "9px", letterSpacing: "3px", textTransform: "uppercase", transition: "all .2s", fontWeight: 500 }}
                >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
              ...
              <input
                ...
                onKeyDown={(e) => e.key === "Enter" && (lmode === "login" ? handleLogin() : handleRegister())}
              />

              {lerr && (
                <div style={{ color: lerr.startsWith("✅") ? "#2ecc71" : "#e74c3c", fontSize: "12px", textAlign: "center", lineHeight: 1.5, letterSpacing: ".3px" }}>
                  {lerr}
                </div>
              )}
              ...
              {lmode === "login" && (
                <button onClick={handleForgotPassword} disabled={aloading} style={{ background: "none", border: "none", color: "#3a3632", fontSize: "10px", cursor: "pointer", letterSpacing: "1.5px", fontFamily: "'Jost', sans-serif", textDecoration: "underline", textAlign: "center", marginTop: "4px" }}>
          ...
          <div style={{ textAlign: "center", marginTop: "28px", fontSize: "9px", color: "#1e1e1e", letterSpacing: "2px" }}>
            STYLEVAULT © 2025
          </div>
        </div>
      </div>
    ...
    return (
      <div className="sv fade">
        ...
        <main style={{ padding: "20px 16px", maxWidth: "680px", margin: "0 auto" }}>
          {adminL ? (
            <div style={{ display: "flex", justifyContent: "center", gap: "6px", padding: "60px" }}>
            ...
            <>
              {adminTab === "overview" && (
                <div className="fade">
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "20px" }}>
                    {[
                      { label: "Activos", value: actives.length, color: "#2ecc71" },
                      { label: "Premium", value: premiums.length, color: "#C4973F" },
                      { label: "Bloqueados", value: blocked.length, color: "#e74c3c" },
                      { label: "Ingresos/mes", value: `$${(premiums.length * 299).toLocaleString()} MXN`, color: "#60a5fa" },
                    ].map((s) => (
                  ...
                  <div className="card">
                    ...
                    {allUsers.filter((u) => u.plan !== "Admin").slice(0, 5).map((u) => (
                      <div key={u.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "11px 0", borderBottom: "1px solid #141414" }}>
                        <div>
                          ...
                          <div style={{ fontSize: "10px", color: "#3a3632", marginTop: "2px" }}>{u.email}</div>
                        </div>
                    ...
                    {allUsers.length === 0 && <div style={{ fontSize: "12px", color: "#2a2a2a", textAlign: "center", padding: "24px" }}>Sin usuarios aún</div>}
                  </div>
                </div>
              )}
              ...
              {adminTab === "users" && (
                <div className="fade">
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                    {allUsers.filter((u) => u.plan !== "Admin").map((u) => (
                      <div key={u.id} className="card">
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "12px" }}>
                          <div>
                            <div style={{ fontSize: "14px", fontWeight: 400 }}>{u.name}</div>
                            <div style={{ fontSize: "11px", color: "#3a3632", marginTop: "3px" }}>{u.email}</div>
                          </div>
                        ...
                        <div style={{ fontSize: "10px", color: "#3a3632", marginBottom: "14px" }}>Registrado: {u.created_at?.split("T")[0]}</div>
                        <div style={{ display: "flex", gap: "8px" }}>
                          <button className={u.status === "blocked" ? "btn-success" : "btn-danger"} onClick={() => toggleBlock(u)}>
                    ...
                    {allUsers.filter((u) => u.plan !== "Admin").length === 0 && (
                      <div style={{ textAlign: "center", padding: "48px", color: "#2a2a2a", fontSize: "13px" }}>Sin usuarios aún.</div>
                    )}
                  </div>
                </div>
              )}
              ...
              {adminTab === "subscriptions" && (
                <div className="fade">
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "20px" }}>
                    <div className="stat-card" style={{ borderColor: "rgba(196,151,63,.15)" }}>
                      <div style={{ fontSize: "9px", color: "#C4973F", letterSpacing: "2px", marginBottom: "10px", textTransform: "uppercase" }}>Premium</div>
                      <div className="serif" style={{ fontSize: "32px", color: "#C4973F", fontWeight: 300 }}>{premiums.length}</div>
                      <div style={{ fontSize: "10px", color: "#3a3632", marginTop: "6px" }}>$299 MXN/mes c/u</div>
                    </div>
                    <div className="stat-card">
                      <div style={{ fontSize: "9px", color: "#2ecc71", letterSpacing: "2px", marginBottom: "10px", textTransform: "uppercase" }}>Ingresos</div>
                      <div className="serif" style={{ fontSize: "28px", color: "#2ecc71", fontWeight: 300 }}>${(premiums.length * 299).toLocaleString()}</div>
                      <div style={{ fontSize: "10px", color: "#3a3632", marginTop: "6px" }}>MXN este mes</div>
                    </div>
                  </div>

                  <div className="card">
                    <div style={{ fontSize: "9px", color: "#C4973F", letterSpacing: "3px", textTransform: "uppercase", marginBottom: "16px" }}>✦ Detalle</div>
                    {allUsers.filter((u) => u.plan !== "Admin").map((u) => (
                      <div key={u.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0", borderBottom: "1px solid #141414" }}>
                        <div>
                          <div style={{ fontSize: "13px" }}>{u.name}</div>
                          <div style={{ fontSize: "10px", color: "#3a3632", marginTop: "2px" }}>{u.email}</div>
                        </div>
                        <div style={{ textAlign: "right" }}>
                          <span className={`badge-plan ${u.plan === "Premium" ? "badge-premium" : "badge-basic"}`}>{u.plan}</span>
                          <div style={{ fontSize: "10px", color: "#3a3632", marginTop: "5px" }}>{u.plan === "Premium" ? "$299 MXN/mes" : "Gratis"}</div>
                        </div>
                      </div>
                    ))}
                  </div>
  ...
  return (
    <div className="sv">
      ...
      <nav className="tab-nav">
        {[["wardrobe", "Armario"], ["outfit", "Outfit IA"], ["shop", "Tiendas"], ["advisor", "Asesor IA"]].map(([k, l]) => (
          ...
          <div className="fade">
              ...
              <div className="card card-gold fade" style={{ marginBottom: "20px" }}>
                ...
                <div style={{ marginBottom: "16px" }}>
                  <div className="photo-box" style={{ height: "160px" }} onClick={() => fileRef.current?.click()}>
                    {pp ? (
                      <img src={pp} alt="preview" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                    ) : (
                      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "8px", color: "#3a3632" }}>
                        <div style={{ fontSize: "32px" }}>📷</div>
                        <div style={{ fontSize: "10px", letterSpacing: "1.5px", textTransform: "uppercase" }}>Agregar foto</div>
                      </div>
                    )}
                  </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                  ...
                  <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
                    ...
                    <span style={{ fontSize: "12px", color: "#3a3632", letterSpacing: ".5px" }}>Color principal de la prenda</span>
                  </div>
            ...
            {cloading ? (
              ...
              <div style={{ textAlign: "center", padding: "70px 20px" }}>
                ...
                <div style={{ fontSize: "12px", color: "#1e1e1e", letterSpacing: "1px" }}>
                ...
                {filtered.map((item) => (
                  <div key={item.id} className="card" style={{ padding: 0, overflow: "hidden", position: "relative" }}>
                    <button
                      ...
                      style={{ position: "absolute", top: "8px", right: "8px", zIndex: 2, background: "rgba(0,0,0,.7)", border: "none", color: "#7a7269", cursor: "pointer", fontSize: "10px", width: "22px", height: "22px", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", transition: "color .2s" }}
                    >✕</button>
                    {item.photo_url ? (
        ...
        {tab === "outfit" && (
          <div className="fade">
            <div style={{ marginBottom: "24px" }}>
              ...
              <div style={{ fontSize: "11px", color: "#3a3632", marginTop: "4px", letterSpacing: ".5px" }}>La IA selecciona las prendas ideales de tu armario</div>
            </div>
              ...
              <>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px" }}>
                    {EVENTS.map((ev) => (
                      <button
                        key={ev}
                        onClick={() => setSelEv(ev)}
                        style={{ padding: "10px 8px", background: selEv === ev ? "rgba(196,151,63,.1)" : "#0e0e0e", color: selEv === ev ? "#C4973F" : "#3a3632", border: `1px solid ${selEv === ev ? "rgba(196,151,63,.3)" : "#1c1c1c"}`, borderRadius: "1px", cursor: "pointer", fontFamily: "'Jost', sans-serif", fontSize: "11px", transition: "all .2s", textAlign: "left", letterSpacing: ".3px" }}
                      >{ev}</button>
                    ))}
                  </div>
                </div>
                ...
                <div className="card" style={{ marginBottom: "18px" }}>
                  ...
                  <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                    {SEASONS.map((s) => (
                      <button key={s} className={`pill ${selSe === s ? "active" : ""}`} onClick={() => setSelSe(s)}>{s}</button>
                    ))}
                  </div>
                </div>
                ...
                {outfitL && (
                  <div style={{ display: "flex", justifyContent: "center", gap: "6px", padding: "24px" }}>
                    <div className="dot" /><div className="dot" /><div className="dot" />
                  </div>
                )}
                ...
                {outfitR && !outfitL && (
                  <div className="fade">
                    ...
                    <div style={{ fontSize: "9px", color: "#C4973F", letterSpacing: "3px", textTransform: "uppercase", marginBottom: "16px" }}>✦ Tu Look</div>
        ...
        {tab === "shop" && (
          <div className="fade">
            <div style={{ marginBottom: "20px" }}>
              ...
              <div style={{ fontSize: "11px", color: "#3a3632", marginTop: "4px", letterSpacing: ".3px" }}>Links a Google Maps, Amazon y tiendas oficiales</div>
            </div>

            {/* Location bar */}
            <div className={`loc-bar ${locSt === "err" ? "loc-err" : ""}`} style={locSt === "err" ? { borderColor: "rgba(192,57,43,.3)" } : {}}>
              ...
              {locSt !== "ok" && (
                <button className="btn-outline" style={{ flexShrink: 0 }} onClick={requestLoc} disabled={locSt === "loading"}>
                  {locSt === "loading" ? "..." : "Activar"}
                </button>
              )}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px" }}>
                {BUDGETS.map((b) => (
                  <button
                    ...
                    style={{ padding: "12px 10px", background: budget === b.value ? "rgba(196,151,63,.06)" : "#0e0e0e", border: `1px solid ${budget === b.value ? "rgba(196,151,63,.3)" : "#1c1c1c"}`, borderRadius: "1px", cursor: "pointer", textAlign: "left", transition: "all .2s" }}
                  >
                    ...
                    <div style={{ fontSize: "10px", color: "#3a3632", marginTop: "3px", lineHeight: 1.4 }}>{b.desc}</div>
                  </button>
                ))}
              </div>
            </div>
            ...
            {shopR && !shopL && (
              <div className="fade">
                <div className="divider" />
                {shopR.intro && (
                  <div style={{ fontSize: "13px", color: "#7a7269", lineHeight: 1.7, marginBottom: "18px", fontStyle: "italic", borderLeft: "2px solid rgba(196,151,63,.2)", paddingLeft: "14px" }}>
                    {shopR.intro}
                  </div>
                )}
                  ...
                  {shopR.items?.map((item: any, i: number) => (
                    <div key={i} style={{ background: "#0d0d0d", border: "1px solid #1c1c1c", borderRadius: "2px", padding: "16px" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "8px" }}>
                        <div style={{ fontSize: "14px", fontWeight: 400, flex: 1, lineHeight: 1.3 }}>{item.name}</div>
                      ...
                      <div style={{ marginBottom: "12px" }}>
                        <div style={{ fontSize: "9px", color: "#2a2826", letterSpacing: "2px", textTransform: "uppercase", marginBottom: "10px" }}>
                          {locSt === "ok" ? `📍 Cerca de ${loc?.city}` : "🌍 Disponible en"}
                        </div>
                        ...
                        {item.hasAmazon && !item.stores?.includes("Amazon") && (
                          <div style={{ background: "#0a0a0a", border: "1px solid #181818", borderRadius: "2px", padding: "10px 12px", marginBottom: "7px" }}>
                            <div style={{ fontSize: "12px", fontWeight: 400, marginBottom: "8px", color: "#7a7269" }}>Amazon</div>
                            <a href={amazonUrl(item.name)} target="_blank" rel="noopener noreferrer" className="lnk lnk-amazon">📦 Buscar en Amazon</a>
                          </div>
                        )}
                      </div>

                      {item.tip && (
                        <div style={{ background: "#080808", borderLeft: "2px solid rgba(196,151,63,.15)", padding: "9px 12px", borderRadius: "0 2px 2px 0", fontSize: "11px", color: "#3a3632", lineHeight: 1.6, marginBottom: "8px" }}>
                          💡 {item.tip}
                        </div>
                      )}
        ...
        {tab === "advisor" && (
          <div className="fade" style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 175px)" }}>
            <div style={{ marginBottom: "16px" }}>
              ...
              <div style={{ fontSize: "10px", color: "#3a3632", marginTop: "3px", letterSpacing: ".5px" }}>Powered by Claude AI · Historial completo de conversación</div>
            </div>
            ...
            <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: "14px", paddingBottom: "14px" }}>
              {msgs.map((m, i) => (
                <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start", gap: "8px", alignItems: "flex-start" }}>
                  {m.role === "assistant" && (
                    <div style={{ fontSize: "12px", marginTop: "6px", flexShrink: 0, color: "#C4973F", fontFamily: "'Cormorant Garamond', serif" }}>✦</div>
                  )}
              ...
              {cload && (
                <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                  ...
                  <div className="bubble-ai">
                    <div style={{ display: "flex", gap: "5px", padding: "2px 0" }}><div className="dot" /><div className="dot" /><div className="dot" /></div>
                  </div>
                </div>
              )}
              <div ref={chatEnd} />
            ...
            <div style={{ display: "flex", gap: "8px", paddingTop: "14px", borderTop: "1px solid #141414" }}>
              <input
                className="inp"
                style={{ flex: 1 }}
                placeholder="Pregunta sobre moda, outfits, dress codes, tendencias..."
                value={cin}
                onChange={(e) => setCin(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && !cload && sendChat()}
              />
              <button className="btn-outline" style={{ flexShrink: 0 }} onClick={() => sendChat()} disabled={cload || !cin.trim()}>
                Enviar
              </button>
            </div>
          </div>
